#!/usr/bin/env bash
# =============================================================================
# EUDI Wallet - Relying Party (RP) Test Harness & Integration Simulator
#
...
# Es führt folgende Schritte vollautomatisch per curl und jq aus:
# 1. Sitzungs-Initialisierung (GET /api/presentation/initiate)
# 2. Extraktion von Session-ID (state) und der kryptografischen Nonce
# 3. Zusammenbau einer validen direct_post-Payload (Erika Mustermann Mockup)
#    - Signiertes SD-JWT VC (Fälschungssicheres Identity Credential)
#    - Selektiv offengelegte Klartext-Disclosures (Erika, Mustermann, 1998-08-12, über 18, Köln)
#    - Geräteseitiges Key Binding JWT (Holder Binding Proof) bound to Session-Nonce
# 4. Übermittlung des direct_post Callbacks (POST /api/presentation/callback)
# 5. Abfrage des finalen Onboarding-Status im Server (GET /api/presentation/status)
# =============================================================================
...
if ! curl -s --head --request GET "${API_BASE}/api/presentation/initiate" > /dev/null; then
    ...
    echo "Bitte starten Sie zuerst den Server mit: node eudi-verifier-server.js"
    exit 1
fi
